...
open class Banner: UIView {
    ...
    open var minimumHeight: CGFloat = 65
    ...
    public required init(title: String? = nil, subtitle: String? = nil, image: UIImage? = nil, backgroundColor: UIColor = UIColor.black, didTapBlock: (() -> Void)? = nil) {
    ...
    internal func didTap(_: UITapGestureRecognizer) {
        if dismissesOnTap {
            dismiss()
        }
        didTapBlock?()
    ...
    internal func didSwipe(_: UISwipeGestureRecognizer) {
        if dismissesOnSwipe {
            dismiss()
        }
    }
    ...
    private func initializeSubviews() {
        ...
        let slash = "H:"
        let constraintFormat = slash + "\(leftConstraintText)-(15)-[labelView]-(8)-|"
    ...
    public required init?(coder _: NSCoder) {
        ...
        LogManager.logSevere("init(coder:) has not been implemented in Banner")
